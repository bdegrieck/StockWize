        ...
        <>
            <div className="d-flex flex-column col-3 vh-100 position-absolute pt-3">
                ...
                <div className="flex-grow-1 d-flex flex-column col-11 rounded-end grey align-items-center">
                    <NavBarItem route="/content/overview" company={company} img={Binoculars} text="Overview"/>
                    <NavBarItem route="/content/historical" company={company} img={Clock} text="Historical"/>
                    <NavBarItem route="/content/forecasted" company={company} img={Ball} text="Forecasted"/>
                    <NavBarItem route="/content/stocknews" company={company} img={News} text="Stock News"/>
                    <NavBarItem route="/content/explanatory" company={company} img={Eye} text="Explanatory"/>
                    <NavBarItem route="/content/microeconomics" company={company} img={Money} text="Microeconomics"/>
                    <NavBarItem route="/content/compare" company={company} img={Scale} text="Compare"/>
...
function NavBarItem({ route, company, img, text } : { route: string, company: string | null, img: StaticImageData, text: string }) {
    const href = company === null ? route : `${route}?company=${company}`
    const pathName = usePathname()
    const background_color = pathName === route ? "#E8E8E8" : "clear"
    return (
        <Link href={href} className="mt-2 w-100 d-flex align-items-center justify-content-center" style={{backgroundColor: background_color}}>
