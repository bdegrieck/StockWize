...
from BackEnd.error import ArrayShapeError
...
class ConfidenceIntervalBounds(BaseModel):
    ...
    def check_bounds_ndim(cls, bounds):
        if bounds.ndim != 1:
            raise ArrayShapeError(array_size=bounds.ndim, allowed_size=1)
        return bounds
...
class CorrelationData(BaseModel):
    ...
    def check_confidence_interval_shape(cls, coef):
        if coef.ndim != 1:
            raise ArrayShapeError(array_size=coef.ndim, allowed_size=1)
        return coef
